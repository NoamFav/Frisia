...
public class CaptureTest {
    ...
    public void testCapturingMoveInitialization() {
        ...
        assertEquals(
                start,
                capturingMove.getStartPosition(),
                "Start position should be initialized correctly.");
        assertEquals(
                end,
                capturingMove.getEndPosition(),
                "End position should be initialized correctly.");
        assertEquals(
                captured,
                capturingMove.getCapturedPositions(),
                "Captured positions should be initialized correctly.");
        assertTrue(
                capturingMove.isCapture(), "isCapture() should return true for capturing moves.");
    }
    ...
    public void testSingleCaptureMove() {
        ...
        assertEquals(
                1,
                capturingMove.getCapturedPositions().size(),
                "There should be exactly one captured position.");
        assertEquals(
                captured,
                capturingMove.getCapturedPositions(),
                "Captured positions should match the input.");
    }
    ...
    public void testMultipleCapturesMove() {
        ...
        assertEquals(
                3,
                capturingMove.getCapturedPositions().size(),
                "There should be three captured positions.");
        assertEquals(
                captured,
                capturingMove.getCapturedPositions(),
                "Captured positions should match the input.");
    ...
    public void testNonCapturingMoveIsNotCapture() {
        ...
        // Assert
        assertFalse(
                nonCapturingMove.isCapture(),
                "Non-capturing move should not be identified as a capturing move.");
        assertTrue(
                nonCapturingMove.getCapturedPositions().isEmpty(),
                "Captured positions should be empty for non-capturing move.");
    }
